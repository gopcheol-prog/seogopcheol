...
(function () {
  ...
  var css = `
    ...
    /* 프사 클릭 바운스(눌리는 반응) */
    .fx-avbounce{ animation:fxAvBounce .5s cubic-bezier(.34,1.56,.64,1); }
    @keyframes fxAvBounce{ 0%{ transform:scale(1); } 22%{ transform:scale(.86); } 48%{ transform:scale(1.1); } 72%{ transform:scale(.96); } 100%{ transform:scale(1); } }
    @media (prefers-reduced-motion: reduce){ #fx{ display:none; } .card{ transition:none; } .fx-heart{ display:none; } .fx-avbounce{ animation:none; } }
  ...
  function build() {
    ...
    if (av && !av.dataset.fxPop) {
      ...
      var avImg = av.matches('.avatar') ? av : (av.querySelector('.avatar') || av);
      av.addEventListener('click', function (e) {
        window.fxHearts(e.clientX, e.clientY, 10);
        if (!mqReduce && avImg) { avImg.classList.remove('fx-avbounce'); void avImg.offsetWidth; avImg.classList.add('fx-avbounce'); }
      });
      if (avImg) avImg.addEventListener('animationend', function (ev) { if (ev.animationName === 'fxAvBounce') avImg.classList.remove('fx-avbounce'); });
    }
  }
